...
shm_error_t shm_create(const char* name,
                       size_t data_size,
    ...
    if (validate_name(name) != 0) {
        return SHM_ERR_NAME_INVALID;
    }
    if (data_size == 0) {
        return SHM_ERR_SIZE_INVALID;
    }
    if (!out_handle) {
    ...
    if (fd < 0) {
        if (errno == EEXIST) return SHM_ERR_ALREADY_EXISTS;
        if (errno == ENOENT) return SHM_ERR_NOT_FOUND;
    ...
    if (flags & SHM_FLAG_READONLY) {
        prot = PROT_READ;
    }

    void* addr = mmap(NULL, total_size, prot, MAP_SHARED, fd, 0);
    if (addr == MAP_FAILED) {
        close(fd);
        shm_unlink(name);
        return SHM_ERR_MAP_FAILED;
    }
    ...
    header->lock.owner = getpid();
    header->lock.state = SHM_LOCK_FREE;

    shm_handle_t* handle = malloc(sizeof(shm_handle_t));
    if (!handle) {
        close(notify_fd);
        munmap(addr, total_size);
        close(fd);
    ...
    handle->fd = fd;
    handle->addr = addr;
    handle->mapped_size = total_size;
    handle->is_producer = 1;
    handle->local_notify_fd = notify_fd;
...
shm_error_t shm_join(const char* name,
                     shm_permission_t perm,
                     shm_handle_t** out_handle) {
    (void)perm;
    ...
    handle->local_notify_fd = dup(header->notify_fd);
    handle->owner_thread = pthread_self();
...
shm_error_t shm_unmap(shm_handle_t* handle) {
    ...
    if (munmap(handle->addr, handle->mapped_size) < 0) {
        return SHM_ERR_UNMAP_FAILED;
    }
    handle->addr = NULL;
    return SHM_OK;
}
...
shm_error_t shm_close(shm_handle_t** handle) {
    ...
    if (impl->is_producer && header && header->magic == SHM_MAGIC) {
        if (impl->local_notify_fd >= 0) close(impl->local_notify_fd);
        close(impl->fd);
        shm_unlink(impl->name);
    } else {
...
shm_error_t shm_wait(shm_handle_t* handle, int timeout_ms) {
    ...
    if (ret == 0) return SHM_ERR_TIMEOUT;
    return (pfd.revents & POLLIN) ? SHM_OK : SHM_ERR_SYSTEM;
}

int shm_get_notify_fd(shm_handle_t* handle) {
    if (!handle) return -1;
    return handle->local_notify_fd;
}

shm_error_t shm_consume_notify(shm_handle_t* handle) {
    if (!handle) return SHM_ERR_INVALID_PARAM;
    eventfd_t val;
    if (eventfd_read(handle->local_notify_fd, &val) < 0) return SHM_ERR_SYSTEM;
    return SHM_OK;
}
...
shm_error_t shm_lock(shm_handle_t* handle, int timeout_ms) {
    ...
    if (ret == EOWNERDEAD) {
        ...
        header->lock.owner = getpid();
        header->lock.state = SHM_LOCK_BUSY;
...
shm_error_t shm_unlock(shm_handle_t* handle) {
    ...
    header->lock.state = SHM_LOCK_FREE;
    header->lock.owner = 0;
    return (pthread_mutex_unlock(&header->lock.mutex) == 0) ? SHM_OK : SHM_ERR_SYSTEM;
}

bool shm_is_locked(shm_handle_t* handle) {
    if (!handle) return false;
    shm_header_t* header = (shm_header_t*)handle->addr;
    return header->lock.state == SHM_LOCK_BUSY;
}
